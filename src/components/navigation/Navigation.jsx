      ...
      <ResponsiveComponent>
        {({ size }) => {
          return size && size >= 480 ? (
            ...
            <>
              <div className="w-full px-2.5 xs:p-0 xs:w-max flex flex-col space-y-5 items-start xs:items-center justify-center relative group">
                {BtnList.slice(0, BtnList.length / 2 + 1 ).map((btn, index) => {
                
                  return <NavButton key={btn.label} x={0} y={0} {...btn} />;
                })}
              </div>
              <div className="w-full px-2.5 xs:p-0 xs:w-max flex flex-col space-y-5 items-end xs:items-center justify-center relative group">
                {BtnList.slice(BtnList.length / 2+1, BtnList.length).map((btn, index) => {
                 
                  return <NavButton key={btn.label} x={0} y={0} {...btn} lableDirection="left" />;
                })}
              </div>
            </>
          );
